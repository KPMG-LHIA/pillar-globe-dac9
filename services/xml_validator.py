...
Changelog v1.3.4 (2026-07):
  CONFRONTO CON REPORT UFFICIALE ADE (file IT_07224860960.xml, GIR26):
  70033   – fix severità: TIN=OtherUPE/ExcludedUPE è W7033 (Warning AdE), non KO.
              Verificato su 18/18 occorrenze nel report ufficiale AdE.
  70079   – fix tolleranza (era >1, ora !=0 → allineata ad AdE) e severità
              (era KO, ora WARN come W7079 AdE). Verificato su giurisdizione SB.
  70086   – idem 70079 (W7086 AdE). Verificato su giurisdizione SB.
  70121   – fix severità: era KO, ora WARN come W7121 AdE.
              Verificato su 3/3 occorrenze (PA/SG/SB) nel report ufficiale AdE.
  _warn_all() – nuovo helper (analogo a _ko_all ma imposta STATUS_WARN).
...
def _warn_all(r: "CheckResult", bad: list, max_items: int = 50) -> "CheckResult":
    """
    Come _ko_all ma imposta r come WARN invece di KO.
    Usato per i check dove l'esito reale AdE è "W" (Warning, non bloccante)
    anziché "Error" — confermato empiricamente confrontando l'output del
    validatore con il report ufficiale AdE per codici 70033/70079/70086/70121.
    """
    if not bad:
        return r
    items = bad[:max_items]
    suffix = f"\n… (+{len(bad)-max_items} altri)" if len(bad) > max_items else ""
    r.status = CheckResult.STATUS_WARN
    r.detail = "\n".join(items) + suffix
    return r
...
def _check_other(root):
    ...
    # CONFERMATO su report ufficiale AdE (file IT_07224860960.xml, 18/18 occorrenze
    # corrispondenti): quando il TIN è quello di OtherUPE/ExcludedUPE, AdE lo
    # segnala come "W7033" (Warning, non bloccante), non come errore. Solo il
    # caso "TIN non corrisponde a nessun CE né UPE" resta KO.
        ...
        for ce in _findall(cs,"globe:CE",ns):
            ...
            for exc in _findall(q,"globe:Exception",ns):
                ...
                if tv==own:
                    ...
                    # TIN è della UPE: AdE lo segnala come W7033 (Warning), non blocca la trasmissione
                    bad_warn.append(f"Exception/TIN={tv!r} corrisponde a OtherUPE/ExcludedUPE, non a un CE (W7033 AdE, non bloccante){_src(te)}")
...
def _check_defer_tax(jur_sections, ns):
    ...
    # 70079
    # CONFERMATO su report ufficiale AdE (file IT_07224860960.xml, giurisdizione SB):
    # AdE flagga come W7079 anche uno scostamento di 1 unità (arrotondamento);
    # la tolleranza precedente (>1) lo escludeva erroneamente (falso negativo).
    # Severità allineata ad AdE: W7079 = Warning, non Error.
    ...
    for js in jur_sections:
        ...
        for oc in _findall(js,".//globe:ETRComputation/globe:OverallComputation",ns):
            ...
            if pre!=(bef+tadj): bad.append(f"{jn}: PreRecast={pre} ≠ {bef}+{tadj}={bef+tadj}{_src(pe)}")
    _warn_all(r, bad)
    out.append(r)
...
def _check_excess(jur_sections, ns):
    ...
    # 70086
    # CONFERMATO su report ufficiale AdE (file IT_07224860960.xml, giurisdizione SB):
    # stesso pattern di 70079 — scostamento di 1 unità flaggato da AdE come W7086.
    # Tolleranza stretta + severità WARN allineate al comportamento reale AdE.
    ...
    for js in jur_sections:
        ...
        for oc in _findall(js,".//globe:ETRComputation/globe:OverallComputation",ns):
            ...
            if ep!=exp: bad.append(f"{jn}: ExcessProfits={ep} ≠ max(0,{ngi}-{se})={exp}{_src(ee)}")
    _warn_all(r, bad)
    out.append(r)
...
def _check_ce_ngi(jur_sections, ns):
    ...
    # 70120
    # BUG STRUTTURALE CONFERMATO su file IT_07224860960.xml (3/3 falsi positivi,
    # AdE non li segnala affatto): Recast è annidato dentro ogni singolo
    # Adjustment (Adjustment/Recast/Higher-Lower), non è figlio diretto di
    # DeferTaxAdjustAmt. Il vecchio XPath "globe:Recast/globe:Higher" cercava
    # Recast al livello sbagliato e restituiva sempre 0 → totale calcolato errato.
    r=CheckResult("70120","CEComputation/DeferTaxAdjustAmt/Total = DeferTaxExpense + Σ(Adjustment/Amount) + Σ(Adjustment/Recast/Higher) + Σ(Adjustment/Recast/Lower).","JurisdictionSection/.../CEComputation/AdjustedCoveredTax/DeferTaxAdjustAmt/Total")
    bad=[]
    for js in jur_sections:
        jn=_t(_find(js,"globe:Jurisdiction",ns))
        for ce in _findall(js,".//globe:CEComputation",ns):
            ...
            adjs=_findall(dta,"globe:Adjustment",ns)
            as_=sum((_decimal(_t(_find(adj,"globe:Amount",ns))) or Decimal(0)) for adj in adjs)
            hi=sum((_decimal(_t(_find(adj,"globe:Recast/globe:Higher",ns))) or Decimal(0)) for adj in adjs)
            lo=sum((_decimal(_t(_find(adj,"globe:Recast/globe:Lower",ns))) or Decimal(0)) for adj in adjs)
            if tot is None: continue
    ...
    for js in jur_sections:
        ...
        for ec in _findall(js,".//globe:ETRComputation",ns):
            ...
            if dups:
                ...
                bad.append(f"{jn}: DeferTaxAdjustAmt/Adjustment AdjustmentItem duplicati tra CEComputation: {dups} (W7121 AdE, non bloccante){_src(first_dup_el)}")
    # CONFERMATO su report ufficiale AdE (file IT_07224860960.xml, 3/3 occorrenze
    # corrispondenti: PA/SG/SB): W7121 = Warning, non Error.
    _warn_all(r, bad)
    out.append(r)

    # 70122
    # 70122 FIX: stesso bug già risolto per 70114/70118 — "segni opposti" va
    # verificato solo tra Amount con lo STESSO AdjustmentItem code, non su
    # una coppia qualsiasi. Confermato su file IT_07224860960.xml: i 2 falsi
    # positivi (giurisdizione SB) confrontavano un Amount di GIR2504 con uno
    # di GIR2516 (codici diversi, mai da confrontare) e uno dei due era 0
    # (né positivo né negativo) solo perché quell'Adjustment porta un Recast.
    r=CheckResult("70122","Se due valori CEComputation/DeferTaxAdjustAmt/Adjustment/Amount con stesso AdjustmentItem, uno deve essere negativo e l'altro positivo.","JurisdictionSection/.../CEComputation/AdjustedCoveredTax/DeferTaxAdjustAmt/Adjustment/Amount")
    bad=[]
    for js in jur_sections:
        jn=_t(_find(js,"globe:Jurisdiction",ns))
        for ce in _findall(js,".//globe:CEComputation",ns):
            for code,pairs in _adj_groups(ce,"globe:AdjustedCoveredTax/globe:DeferTaxAdjustAmt/globe:Adjustment",ns).items():
                if len(pairs)==2:
                    a0,e0=pairs[0]; a1,e1=pairs[1]
                    if not (a0<0<a1) and not (a1<0<a0):
                        bad.append(f"{jn}: AdjustmentItem={code!r} ha due Amount [{a0},{a1}] senza segni opposti{_src(e0)}{_src(e1)}")
